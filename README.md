# Web/DA — Умный реестр контента в MWS Tables
Проект, созданный в рамках хакатона.  
Web/DA — это веб-приложение для анализа эффективности контента с использованием данных из MWS Tables и возможностей LLM.
Приложение позволяет:
- просматривать сводную статистику контента,
- анализировать метрики публикаций,
- сравнивать посты между собой,
- получать текстовые инсайты через LLM,
- изучать детали каждой публикации,
##Демо функциональности
- Dashboard — ключевые метрики: просмотры, лайки, комментарии, ER, sentiment.  
- Реестр контента — таблица со всеми публикациями, быстрые метрики, карточка деталей.  
- Сравнение — сравнение эффективности выбранных постов.  
- LLM-чат — диалог с моделью для получения аналитических инсайтов.  
- Источники — подключение внешних API (MWS Tables и др.).
---
##Команда проекта
- @splend1ddd — Frontend
Ведёт внешний облик проекта и его логику на стороне клиента. Создаёт интерфейсы, которые работают плавно и выглядят так, будто рождены для света.
“Ars est celare artem” — искусство скрывать искусство.
- @darlinglis0  — Designer
Ответственна за визуальный стиль, UX-паттерны и гармонию. Добавляет проекту глубину и узнаваемость, превращая сухие требования в выразимые формы.
- @seele_supremacy  — Data Analyst
Работает с аналитикой, метриками, выводами и здравым смыслом. Превращает данные в решения, а хаос — в структуру.
- @lolkries — Product
Следит за логикой продукта, сценарием использования, пользовательскими потоками. Задает направление и помогает проекту расти в нужную сторону.
- @valua_s  — Backend
Создаёт архитектуру, API, бизнес-логику, агентный модуль, orchestrates сервисы и следит, чтобы всё работало надёжно.
“Fortis fortuna adiuvat” — удача любит храбрых.
---
Технологии
### Frontend
- React (Vite)
- Axios
- CSS 
- SVG-иконки

### Backend
- Python
- FastAPI
- Pydantic
---
## Как запустить проект
Проект запускается полностью через Docker, без необходимости вручную поднимать backend и frontend.
### Запуск
```bash
docker compose up --build

## После сборки и запуска сервисов приложение будет доступно по адресу:

http://localhost/
